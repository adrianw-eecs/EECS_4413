...
//		System.out.println(request.getPathInfo());
		if(request.getPathInfo() != null && request.getPathInfo().contains("/Fixed")) {

			if(submitQuery) {
				String pathInfo = request.getPathInfo();
				String letter = pathInfo.substring(pathInfo.length()-1, pathInfo.length());
...
//				getDefaultWebXML(request);
				...
				if(letter.matches("[A-F]")) {
					period = "10";
				} else if(letter.matches("[G-K]")) {
					period = "15";
				} else if(letter.matches("[L-O]")) {
					period = "20";
				} else if(letter.matches("[P-Z]")) {
					period = "30";
				}
				System.out.println("Period is:" + period);
				
				interest = extractFromQueryString(request, "radioInterest"); 
				System.out.println("->|" + principal  + "|" + interest  + "|" + period  + "|" + grace);
				computeMonthly();
				request.getSession().setAttribute("fixedInterest", interest);
				request.getSession().setAttribute("fixedPeriod", period);
				request.getSession().setAttribute("letterValue", letter);
				setResultsAttributes(request);
				request.getSession().setAttribute("hideGrace", 1);
				...
				request.getSession().setAttribute("hidePeriod", 1);
				target = "/UI.jspx";
				request.getRequestDispatcher(target).forward(request,response);
			}
		}
//		searchs for /Ajax
		else if(request.getPathInfo() != null && request.getPathInfo().contains("/Ajax")){
...
//			System.out.println("abc" + graceVal);
		...
		else if(submitQuery && !(request.getPathInfo() != null && request.getPathInfo().contains("/Fixed"))) {
			getDefaultWebXML(request);
			extractAllQueryString(request);
//			System.out.println("|" + principal  + "|" + interest  + "|" + period  + "|" + grace);
			setUIAttributes(request);
	...
	private void setUIAttributes(HttpServletRequest request) {
		request.getSession().setAttribute("hidePeriod", 0);
		request.getSession().setAttribute(principalSVD, principal);
		request.getSession().setAttribute(principalSVD, principal);
		request.getSession().setAttribute(interestSVD, interest);
		request.getSession().setAttribute(periodSVD, period);
	...
	private void setResultsAttributes(HttpServletRequest request) {
		request.getSession().setAttribute("hideGrace", 0);
